# Neo Matrix —— 把大家的闲置 API Key，汇成一个能分钱的入口

> 一个基于 one-api（MIT）改造的共享 AI 中转站：消费者拿一个 `sk-` 令牌用遍所有大模型；供给方把闲置 API Key 托管进来，按真实用量分成、可提现。**关键机制是调度算法和结算对账，它们决定钱怎么分、分给谁、会不会分错。**

![结算与提现管理（核心页面）](images/05-settlement.png)

上面是平台管理端的结算页：12 张结算单、¥123 流水、6 张已入账、1 张"对账异常"等着核验。这篇文章讲它是怎么做到的——不是讲 UI，是讲**背后的调度与结算机制**。

## 为什么会有这个项目

大家手里的 API Key 通常是分散的：OpenAI、DeepSeek、Gemini、各种聚合平台…… 用的时候要在服务商之间切换、分别计费、分别管余额。而另一方面，很多人的 Key 是**闲置**的——包月订阅用不完，或者测试 Key 吃灰。

Neo Matrix 的想法很简单：**把这些 Key 汇总成一个入口**。

- **消费者**：只认一个 `sk-` 令牌，所有模型一个地方按量付费。
- **供给方**：闲置 Key 托管进来，按真实消耗分成，能提现。
- **平台**：同模型多渠道时自动走成本最优的渠道，把利润做大。

听起来像"聚合中转站"的老路子？区别在于**分钱是透明的、可对账的**——供给方拿多少、平台留多少，由调度和结算机制精确决定，而不是拍脑袋。
...
**消费者拿到的 key，是平台发的令牌，不是任何上游的 key。** 请求打到平台后，**调度算法决定交给哪个供给方的 Key 去处理**——消费者无感知，也决定不了用哪家。

## 架构：调度 + 结算两条链路

![Neo Matrix 调度与结算架构](images/00-architecture.png)

平台上有四个模块在协作：**成本最优路由**决定请求走哪个渠道；**消费记账**同时记零售价和成本价；**分成结算**按周期把利润分给供给方；**对账 + 信任**保证账目可信、异常渠道被压制。

## 调度算法：这个请求到底给谁？

同模型有多个渠道时，**不是随机派发，也不是只挑最便宜的**，而是按 `1 / (成本倍率 × 信任惩罚)` 加权随机：
...
2. 组内按 `WeightFactor = 1 / (CostRatio × trustPenalty)` **加权随机**：
   ...
   - **低信任渠道概率非零**——保证新渠道能"爬坡"
3. **反套利防线**：如果消费者正好是某渠道的供给方，调度会**排除他自己托管的渠道**，从机制上杜绝"自产自销套分成"。
...
- **自动爬坡**:连续 7 个周期**对账正常** → 信任自动 +1(封顶 5)。新渠道靠真实运营自动获得更高调度权重,不用等管理员手动提。
- **管理员手动封顶/干预**:成本申报审批时可顺带指定信任等级(1-5)。
- **异常自动降权**:连续 2 个周期对账异常 → 信任自动 -1(最低 1),只降不升。
...
![结算单（含对账异常）](images/05-settlement.png)
...
供给方中心一眼看清账目：可提现余额 ¥54.06、结算中 ¥63.05、平台抽成 20%。
